...
//define
#define ctrl(c) ((c) & 0x1f)     // mask first 5 bits to obtain ctrl(c) value
#define toLower(c) ((c) | 0x20)  // set the 5th, 1 to obtain lowerCase of the c
#define toUpper(c) ((c) & ~0x20) // set the 5th, 0 to obtain lowerCase of the c
...
void rawMode() {
    ...
    if ( (tcgetattr(STDIN_FILENO, &E.ogTermios)) != 0){
        errorAndExit();
    }
    ...
    struct termios raw = E.ogTermios;
...
void exitRawMode() {
    if ( (tcsetattr(STDIN_FILENO, TCSAFLUSH, &E.ogTermios ) != 0)){
        errorAndExit();
    }
}
int editorReadKey() {
    ...
    while ((nread = read(STDIN_FILENO, &c, 1)) != 1) {
        if (nread == -1 && errno != EAGAIN) errorAndExit();
    }
